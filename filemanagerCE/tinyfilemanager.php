...
// Get the path two levels below the current file location
$gsRoot = dirname(dirname(__DIR__)) . '/';

// Include GetSimple CMS core
require_once($gsRoot . 'gsconfig.php'); // Path to GetSimple's config file
require_once($gsRoot . 'admin/inc/common.php'); // Path to GetSimple's common functions
...
//Application Title
define('APP_TITLE', 'File Manager CE');
...
$favicon_path = '../../admin/apple-touch-icon.png';

// Files and folders to excluded from listing
// e.g. array('myfile.html', 'personal-folder', '*.php', '/path/to/folder', ...)
$exclude_items = array('.htaccess','index.php','admin','backups','themes','plugins','cache','other','pages','posts','users','sitemap.xml','robots.txt','filemanager');
...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
        ...
        echo "Username: " . htmlspecialchars($_POST['fm_usr']) . "<br>";
        echo "Password: " . htmlspecialchars($_POST['fm_pwd']) . "<br>";
        ...
        $userFile = $gsRoot . 'data/users/' . $username . '.xml';
        echo "User file path: " . $userFile . "<br>";

        // Validate user using GetSimple's authentication
        if (file_exists($userFile)) {
            $userData = getXML($userFile);
            $storedPassword = (string)$userData->PWD;

            // Verify the password using SHA1
            if (sha1($password) === $storedPassword && verifyToken($_POST['token'])) {
    ...
    <div class="row">
        <div class="col-12">
            <ul class="list-group w-50 my-3" data-bs-theme="<?php echo FM_THEME; ?>">
                <li class="list-group-item active" aria-current="true"><strong><?php echo lng($view_title) ?>:</strong> <?php echo $file;?></li>
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                <?php endif; ?>
                <a class="fw-bold btn btn-outline-primary" href="<?php echo  str_replace(str_replace('\\','/',GSDATAUPLOADPATH),$SITEURL.'data/uploads/',fm_enc($file_path)) ; ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                <?php
                // ZIP actions
            ...
            <div class="row mt-3">
                ...
                if ($is_onlineViewer) {
                    ...
                    if (in_array($ext, array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))) {
                        echo '<p><input type="checkbox" id="preview-img-zoomCheck"><label for="preview-img-zoomCheck"><img src="' . str_replace(str_replace('\\','/',GSDATAUPLOADPATH),$SITEURL.'data/uploads/',fm_enc($file_path)) . '" alt="image" class="preview-img"></label></p>';
                    }
                } elseif ($is_audio) {
                    // Audio content
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            foreach ($files as $f) {
                ...
                $filesize = fm_get_filesize($filesize_raw);
                $filelink = '?p=' . $_GET['p'] . '&amp;view=' . urlencode($f);
                    ...
                    <td data-sort=<?php echo fm_enc($f) ?>>
                        <div class="filename">
                            ...
                            if (in_array(strtolower(pathinfo($f, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))): ?>
                                <?php $imagePreview = fm_enc(FM_ROOT_URL .'/theme'. (FM_PATH != '' ? '/' . FM_PATH : '') .'/'. $f); ?>
...
?>
    ...
    <head>
		...
		<?php echo '<link rel="icon" href="../../admin/apple-touch-icon.png" type="image/png">';?>
...
?>
    ...
    <head>
        ...
        <meta name="googlebot" content="noindex">
        <?php echo '<link rel="icon" href="../../admin/apple-touch-icon.png" type="image/png">';?>
        <title><?php echo fm_enc(APP_TITLE) ?> | <?php echo (isset($_GET['view']) ? $_GET['view'] : ((isset($_GET['edit'])) ? $_GET['edit'] : "GS-CE")); ?></title>
        <?php print_external('pre-jsdelivr'); ?>
        ...
        <style>
            ...
            *::after {
			...
			.dropdown-menu[data-bs-popper] {left: -100px;}
        </style>
        <?php
        if (FM_THEME == "dark"): ?>
            <style>
